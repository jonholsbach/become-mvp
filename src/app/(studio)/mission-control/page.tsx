...
import { profileToCoachUser } from "@/lib/domain/member";
...
export default function CoachDashboardPage() {
  ...
  const [explorers, setExplorers] = useState<CoachUser[]>(MOCK_COACH_USERS);

  useEffect(() => {
    const profile = loadProfile();
    if (profile) {
      const localUser = profileToCoachUser(
        profile,
        loadAssessment(),
        loadMapUpdate(),
        loadStepChecklist()
      );
      setExplorers([localUser, ...MOCK_COACH_USERS]);
      setSelectedId(localUser.id);
    } else {
      setSelectedId(MOCK_COACH_USERS[0].id);
    }
  }, []);

  const selectedUser = explorers.find((u) => u.id === selectedId) ?? explorers[0];

  const stats = useMemo(
    () => ({
      total: explorers.length,
      needsAction: explorers.filter(
        (u) => u.completionStatus === "reflection_submitted" || u.completionStatus === "not_started"
      ).length,
      inProgress: explorers.filter((u) => u.completionStatus === "in_progress").length,
    }),
    [explorers]
  );
  ...
  return (
    <PageShell
      ...
      backHref="/studio/mission-control"
      maxWidth="6xl"
    >
      <p className="-mt-4 mb-6 text-xs font-medium uppercase tracking-wider text-amber-200/50">
        MBN Studio · Internal only
      </p>
        ...
        <div className="lg:col-span-2">
          ...
          <div className="mt-6 rounded-xl border border-white/5 bg-mbn-panel/40 p-4">
            <p className="text-xs text-mbn-steel-dim">
              {explorers[0]?.id === "local-explorer"
                ? "Your live scan data appears as the first explorer. Additional entries are demo data for coach workflow preview."
                : "Demo data. Run a System Scan to see your live explorer data here."}
            </p>
            <Link
              href="/assessment"
              className="mt-3 inline-block text-xs font-medium text-electric-bright hover:underline"
            >
              {explorers[0]?.id === "local-explorer" ? "Retake System Scan →" : "Run live System Scan →"}
            </Link>
          </div>
        </div>
