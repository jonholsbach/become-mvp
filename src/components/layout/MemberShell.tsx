...
export function MemberShell({ children }: { children: React.ReactNode }) {
  ...
  const hasProfile = progression.stage !== "visitor" && progression.stage !== "scanning";
  const navItems = MEMBER_NAV_ORDER.filter((id) => id !== "dashboard" && id !== "progress");
  const homeHref = progression.universeHome ? routePath("universe") : hasProfile ? routePath("dashboard") : routePath("enter");
    ...
    <div className="member-world min-h-screen pb-20 md:pb-0">
          ...
          <Link href={homeHref} className="flex items-center gap-2">
            ...
            {hasProfile && (
              <div>
                ...
                <p className="text-xs font-medium text-electric-bright">Your world</p>
              </div>
            )}
          </Link>
        ...
        {hasProfile && (
          <div className="border-t border-white/5 bg-mbn-panel/30 lg:hidden">
            <div className="mx-auto flex max-w-6xl gap-1 overflow-x-auto px-2 py-2">
              <MobileNavPill href={routePath("dashboard")} label="Home" active={pathname === routePath("dashboard") || pathname === routePath("universe")} />
      ...
      {hasProfile && (
        <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-white/10 bg-mbn-bg/95 p-3 backdrop-blur-md md:hidden">
          ...
          <p className="mt-1.5 text-center text-[10px] text-mbn-steel-dim">{progression.nextStep.description}</p>
        </div>
      )}
    </div>
  );
