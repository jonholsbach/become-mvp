      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3.5 sm:px-6 sm:py-5">
        <Link href="/" className="group flex items-center gap-2.5 sm:gap-3.5">
          ...
          <div className="hidden sm:block">
            <p className="text-[11px] font-medium uppercase tracking-[0.22em] text-mbn-steel-dim">
        ...
        <nav className="hidden items-center gap-1.5 md:flex">
          {PUBLIC_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="rounded-full px-3.5 py-2 text-sm font-medium text-mbn-steel transition-colors hover:bg-mbn-panel hover:text-mbn-white"
            >
              {link.label}
            </Link>
          ))}
        ...
        <Link href="/enter" className="btn-primary px-5 py-2.5 text-sm sm:px-6 sm:py-3">
          Enter the Body
        </Link>
      </div>
    </header>
